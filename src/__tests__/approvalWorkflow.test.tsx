...
describe("Approval Workflow Test Cases", () => {
  ...
  test("TC-APPROVAL-002: Local state updates immediately after approval", async () => {
    ...
    const TestComponent = () => {
      ...
      const pendingIdeas = data.ideas.filter(
        (idea: any) => idea.status === "Pending Approval",
      );
    ...
    const TestComponent = () => {
      ...
      return React.createElement(
        ...
        React.createElement(
          "div",
          { "data-testid": "last-action" },
          lastAction ? `${lastAction.action}-${lastAction.idea.id}` : "none",
        ),
        React.createElement(
          "button",
          {
            ...
            onClick: () =>
              handleCardAction(
                data.ideas.find((i: any) => i.status === "Pending Approval"),
                "approve",
              ),
          },
          "Approve",
  ...
  test("TC-APPROVAL-014: Undo works for both approve and reject actions immediately", async () => {
    ...
    const TestComponent = () => {
      ...
      const rejectedIdeas = data.ideas.filter(
      ...
      return React.createElement(
        ...
        React.createElement(
          ...
          rejectedIdeas.length.toString(),
        ),
        React.createElement(
          "div",
          { "data-testid": "last-action-type" },
          lastAction ? lastAction.action : "none",
        ),
        React.createElement(
          "button",
    ...
    await waitFor(() => {
      expect(screen.getByTestId("last-action-type")).toHaveTextContent(
        "approve",
      );
      expect(screen.getByTestId("pending-count")).toHaveTextContent("0");
    });
  ...
  test("TC-APPROVAL-015: Cards display properly with local state caching and status condition", () => {
    ...
    expect(
      result.every((idea: any) => idea.status === "Pending Approval"),
    ).toBe(true);
