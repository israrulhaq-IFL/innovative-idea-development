...
const computeAnalytics = (tasks) => {
  ...
  const highPriorityTasks = tasks.filter((t) => t.Priority === 'High' || t.Priority === '(1) High').length
  ...
  return (
    ...
    >
      <div className={styles.kpiHeader}>
        <div className={styles.kpiIcon} style={{ backgroundColor: `${color}0D` }}>
          <i className={icon} style={{ color }}></i>
        </div>
        <div className={styles.kpiTrend}>
          <i className={getTrendIcon(trend)} style={{ color: getTrendColor(trend) }}></i>
        </div>
      </div>
      <div className={styles.kpiContent}>
        <div className={styles.kpiTitle}>{title}</div>
        <div className={styles.kpiValue}>
          <span className={styles.kpiNumber} style={{ color }}>{value}</span>
