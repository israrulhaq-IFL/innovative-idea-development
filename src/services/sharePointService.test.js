...
describe('SharePointService', () => {
  ...
  describe('getUserPermissions', () => {
    it('should mark HOD as view-all and read-only', async () => {
      service.getCurrentUserGroups = vi.fn().mockResolvedValue([{ Title: 'HOD' }])
      ...
      expect(result.allowedDepartments).toEqual(['infra', 'erp', 'ops', 'network'])
    ...
    it('should mark ITG management as view-all but edit-only for dept-manager memberships', async () => {
      service.getCurrentUserGroups = vi.fn().mockResolvedValue([
        { Title: 'ITG Managers' },
        { Title: 'DCI Manager' }
      ])
    ...
    it('should mark department member as view/edit that department only', async () => {
      service.getCurrentUserGroups = vi.fn().mockResolvedValue([{ Title: 'ERP Member' }])
  ...
  describe('getDepartments', () => {
    it('should return all departments for users with canViewAll permission', async () => {
      ...
      expect(result).toEqual([
        { id: 'infra', name: 'Data Center & Cloud Infrastructure', listName: 'si_tasklist', guid: 'e41bb365-20be-4724-8ff8-18438d9c2354' },
        { id: 'erp', name: 'ERP & Software Development', listName: 'erp_tasklist', guid: '4693a94b-4a71-4821-b8c1-3a6fc8cdac69' },
        { id: 'ops', name: 'ITG Operations', listName: 'ops_tasklist', guid: '6eb2cec0-f94f-47ae-8745-5e48cd52ffd9' },
        { id: 'network', name: 'Networks & Security', listName: 'networks_tasklist', guid: '1965d5a7-b9f0-4066-b2c5-8d9b8a442537' }
      ])
    })
  ...
  describe('getTasks', () => {
    it('should fetch tasks from SharePoint API', async () => {
      ...
      expect(result[0]).toHaveProperty('Department', 'Data Center & Cloud Infrastructure')
    })

    it('should filter tasks by department when departmentFilter provided', async () => {
      fetch.mockRejectedValueOnce(new Error('API error'))

      const result = await service.getTasks({ canViewAll: true, allowedDepartments: ['infra', 'erp', 'ops', 'network'] }, 'erp')
    ...
    it('should filter tasks by user permissions', async () => {
      ...
      const permissions = { canViewAll: false, allowedDepartments: ['network'] }
      const result = await service.getTasks(permissions)

      expect(result.every(task => permissions.allowedDepartments.includes(task.DepartmentId))).toBe(true)
    })
  })
